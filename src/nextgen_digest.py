...
Remove companies.json and update logic
Sep 12
·
SC1985/company-intelligence-automation
·
main
+19
-163
...
* Delete `data/companies.json` and keep `data/watchlist.json` as authoritative.
* In `src/nextgen_digest.py`, update `_load_entities` (around line 19) to read `watchlist.json`, skip objects lacking a `"symbol"` key, and include `"asset_class"` in the returned dictionaries.
* Replace any log messages mentioning `companies.json` with `watchlist.json` and ensure downstream logic uses the new data format.
Copy
Summary
Consolidated entity loading to use data/watchlist.json, filtering out entries missing a symbol and ensuring each entity includes an asset_class field

Updated processing logic and logs to reflect watchlist.json as the authoritative source
