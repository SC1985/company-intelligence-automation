...
def _chip(label: str, value: Any, responsive_class: str = "") -> str:
    ...
    class_attr = f' class="{responsive_class}"' if responsive_class else ''
    
    return (
        '<span' + class_attr + ' style="background:' + bg + ';color:' + color + ';padding:5px 12px;'
...
def _build_asset_card(c: Dict[str, Any], is_full_width: bool = False) -> str:
    ...
    # Build chips with responsive classes
    # When full width: display inline (single row)
    # When in columns: use chip-wrapper for 2x2 grid
    if is_full_width:
        # Single row layout for full width cards
        chips = (
            _chip('1D', c.get('pct_1d'), 'chip-responsive') + 
            _chip('1W', c.get('pct_1w'), 'chip-responsive') + 
            _chip('1M', c.get('pct_1m'), 'chip-responsive') + 
            _chip('YTD', c.get('pct_ytd'), 'chip-responsive')
        )
        chips_html = '<tr><td style="margin:12px 0;">' + chips + '</td></tr>'
    else:
        # 2x2 grid for column cards
        chips_html = (
            '<tr><td style="margin:12px 0;">'
...
def _grid(cards_data: List[Dict[str, Any]]) -> str:
    """Arrange cards into a responsive two-column grid."""
    if not cards_data:
        return ''
    rows: List[str] = []
    for i in range(0, len(cards_data), 2):
        left_data = cards_data[i]
        right_data = cards_data[i + 1] if i + 1 < len(cards_data) else None
        
        if right_data:
            # Two column layout
            left = _build_asset_card(left_data, is_full_width=False)
            right = _build_asset_card(right_data, is_full_width=False)
            rows.append(
                '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" '
            ...
            card = _build_asset_card(left_data, is_full_width=True)
            rows.append(
                '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" '
                'style="border-collapse:collapse;margin-bottom:8px;">'
                '<tr><td class="stack-col" style="vertical-align:top;margin:0 auto;">' + card + '</td></tr></table>'
            )
    return ''.join(rows)
...
def render_email(*args: Any, **kwargs: Any) -> str:
    ...
    for sec in ['etf_index', 'equity', 'commodity', 'crypto']:
        ...
        # Pass the card data to _grid, not pre-built HTML
        sec_html = _render_heroes(sec_heroes) + _grid(by_section[sec])
    ...
    css = (
        ...
        '.chip-wrapper { display: inline-block; margin-bottom: 4px; }'
