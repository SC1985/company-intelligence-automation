...
def _chip(label: str, value):
    ...
    return f'<span style="display:inline-block;padding:2px 8px;margin-right:6px;border-radius:12px;background:{bg};color:{color};font-size:12px;">{sign} {txt}</span>'
...
def _heat_square(pct):
    if pct is None:
        ...
        c = f"rgba(52,211,153,{0.25+0.75*mag:.2f})" if pct >= 0 else f"rgba(248,113,113,{0.25+0.75*mag:.2f})"
    return f'<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:{c};margin-right:4px;"></span>'
...
def render_email(summary, companies):
    ...
    for c in companies:
        ...
        spark = f'<img src="data:image/png;base64,{spark_b64}" width="560" height="64" alt="1-month price trend" style="display:block;width:100%;height:auto;border:0;">' if spark_b64 else ""
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin:0 0 12px;background:#111;border:1px solid #2a2a2a;border-radius:8px;">
  <tr>
    <td style="padding:12px 14px;">
      <div style="font-weight:700;font-size:16px;line-height:1.3;color:#fff;">{name} <span style="color:#9aa0a6;">({t})</span></div>
...
"""
    ...
    winners_html = "".join(f'<div style="margin-right:12px;"><strong style="color:#34d399;">▲ {m.get("ticker")}</strong> {m.get("pct",0):+.2f}%</div>' for m in movers_up)
    losers_html = "".join(f'<div style="margin-right:12px;"><strong style="color:#f87171;">▼ {m.get("ticker")}</strong> {m.get("pct",0):+.2f}%</div>' for m in movers_down)
